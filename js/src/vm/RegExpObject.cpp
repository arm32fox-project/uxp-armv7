...
RegExpObject*
RegExpObject::create(ExclusiveContext* cx, const char16_t* chars, 
...
RegExpObject*
RegExpObject::create(ExclusiveContext* cx, HandleAtom source, RegExpFlags flags,
                     TokenStream* tokenStream, LifoAlloc& alloc)
{
...
void
RegExpObject::initIgnoringLastIndex(HandleAtom source, RegExpFlags flags)
{
...
void
RegExpObject::initAndZeroLastIndex(HandleAtom source, RegExpFlags flags, ExclusiveContext* cx)
{
                      ...
                      CompilationMode mode, ForceByteCodeEnum force)
{
    ...
    if (!irregexp::ParsePattern(dummyTokenStream, cx->tempLifoAlloc(), pattern,
                                mode == MatchOnly, this->getFlags(), &data)) {
        return false;
    }
...
bool
RegExpCompartment::get(JSContext* cx, JSAtom* source, RegExpFlags flags, RegExpGuard* g)
{
...
js::XDRScriptRegExpObject(XDRState<mode>* xdr, MutableHandle<RegExpObject*> objp)
{
    ...
    if (!XDRAtom(xdr, &source) || !xdr->codeUint8(&flags))
        return false;
    if (mode == XDR_DECODE) {
        RegExpObject* reobj = RegExpObject::create(xdr->cx(), source, RegExpFlags(flags), nullptr,
 ...
 */
JS_PUBLIC_API(JSObject*) JS::NewRegExpObject(JSContext* cx,
                                             const char* bytes,
                                             size_t length,
                                             RegExpFlags flags) {
  AssertHeapIsIdle(cx);
...
JS_PUBLIC_API(bool) JS::SetRegExpInput(JSContext* cx,
                                       HandleObject obj,
                                       HandleString input) {
  AssertHeapIsIdle(cx);
  CHECK_REQUEST(cx);
...
JS_PUBLIC_API(bool) JS::ExecuteRegExp(JSContext* cx,
                                     HandleObject obj,
                                     HandleObject reobj,
                                     char16_t* chars,
                                     size_t length,
  ...
  return ExecuteRegExpLegacy(cx,
                             res,
                             reobj.as<RegExpObject>(),
                             input,
                             indexp,
                             test,
                             rval);
...
JS_PUBLIC_API(bool) JS::ExecuteRegExpNoStatics(JSContext* cx,
                                               HandleObject obj,
                                               char16_t* chars,
                                               size_t length,
                                               size_t* indexp,
...
JS_PUBLIC_API(bool) JS::ObjectIsRegExp(JSContext* cx,
                                       HandleObject obj,
                                       bool* isRegExp) {
